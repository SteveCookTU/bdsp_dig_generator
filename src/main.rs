...
enum Subcommands {
    Map,
    Results {
        #[clap(help = "Item ID to filter for. Box IDs are 1000 + (type_id * 10) + rarity. rarity is 1 for pretty and 2 for gorgeous")]
        item_id: Option<u32>
    }
}
...
fn main() {
    ...
    match cli.option {
        Subcommands::Map => {
            let result = run_results(Version::BD, rng, cli.diglett, cli.sp_cleared, cli.national_dex, cli.advances, 1);
            ...
            for item in &result.first().unwrap().0 {
                println!("{} - Item ID: {} @ ({},{})", char::from(character), item.0, item.1, item.2);
                character += 1;
            }
        }
        Subcommands::Results { item_id } => {
            let results = run_results(Version::BD, rng, cli.diglett, cli.sp_cleared, cli.national_dex, 0, cli.advances);
